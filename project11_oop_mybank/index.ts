...
// BankAccount interface: Defines the structure of a bank account with account number and balance.
...
class Bank {
  ...
  withdrawCash(accountNumber: number, amount: number): boolean {
    const account = this.accounts.find((acc) => acc.accountNumber === accountNumber);
  ...
  depositCash(accountNumber: number, amount: number): boolean {
    const account = this.accounts.find((acc) => acc.accountNumber === accountNumber);
    if (account) {
      account.balance += amount;
      return true;
